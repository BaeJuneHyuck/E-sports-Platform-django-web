...
class InviteView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        elif TeamRelation.objects.filter(team_pk = self.kwargs.get('pk')).filter(user_pk=User.objects.get(name=invited_user).pk).exists():
            messages.error(self.request, '이미 가입된 멤버입니다.')
            return redirect(self.request.path_info)
        elif TeamInvitation.objects.filter(team_pk = self.kwargs.get('pk')).filter(invited_pk=User.objects.get(name=invited_user).pk).exists():
